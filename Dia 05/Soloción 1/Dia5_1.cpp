...
bool isSortedForRules(vector<int> Linea){
    ...
    for (int i = 0; i < Rules.size(); i++){
        ...
        if ((posFirst > posSecond)&&(posFirst != -1)&&(posSecond != -1)){//si no cumple la regla Finaliza la comprobacion
            cout << "Linea no ordenada: fallo regla "<< Rules[i].first << endl;
            return false;
        }
    }
...
vector<int> ordenarLinea(vector<int> Linea){
    ...
    for (int i = 0; i < Rules.size(); i++){
        for (int j = 0; j < Linea.size(); j++){
            if (Linea[j] == Rules[i].second){
                for (int k = j+1; k < Linea.size(); k++){
                    if (Linea[k] == Rules[i].first){
                        int temp = Linea[j+1];
                        Linea[j+1] = Linea[k];
                        Linea[k] = temp;
                    }
...
int main(int argc, char const *argv[]) {
    ...
    {
        Linea = nextLine(file);
        if (isSortedForRules(Linea)){ SumTotal += getMiddle(Linea);
        }
    }
    
    cout << "Suma de los que cumplen la norma: "<< SumTotal << endl;
